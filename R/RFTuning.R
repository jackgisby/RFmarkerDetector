...
#' 
#' @param expr_df the n x p dataframe used to build the models and to tune the parameter mtry. The first two columns
...
tuneMTRY <- function(expr_df, iterations, maxntree, mtry_length, changeTreeNum = F, graph = T) {

  input <- expr_df[, 3:dim(expr_df)[2]]
  ...
  if (methods::hasArg("iterations") && !is.null(iterations)) {
    iterat <- iterations
  } else {
    iterat <- DEFAULT_ITERATIONS
  }

  if (methods::hasArg("maxntree") && !is.null(maxntree)) {
    nt <- maxntree
  } else {
    nt <- DEFAULT_N_TREES
  }

  if (methods::hasArg("mtry_length") && !is.null(mtry_length) && mtry_length <= numOfVariables) {
...
#' mtry value
#' \item sd_matrix a 1 x p matrix which contains the standard deviation of each OOB error sample resulting from the training of N different models
#' built for each value of mtry}
#'
#' @examples
#' data(cachexiaData)
#' res <- tuneMTRY(cachexiaData[,1:10], iterations = 5, maxntree = 50, mtry_length = 5, graph = FALSE)
#' optimizeMTRY(res$oob)
#' 
#' @author Piergiorgio Palla
#' 
#' @export
...
optimizeMTRY <- function(oob_matrix) {
  ...
  sd_matrix <- apply(oob_matrix, 2, stats::sd)
  ci_matrix <- c()
...
#' 
#' @param expr_df the n x p dataframe used to build the Random Forest models. The first two columns must represent respectively
...
#' @examples
#' data(cachexiaData)
#' tuneNTREE(cachexiaData[,1:10], 8, iterations = 5, minNTREE = 50, pace = 50, seq_length = 10)
#' 
#' @author Piergiorgio Palla
#' 
#' @export

tuneNTREE <- function(expr_df, mtry, iterations, minNTREE = 500, pace = 100, seq_length = 5) {
...
plotOOBvsMTRY <- function(mean_matrix, ci_matrix) {
  ...
  df <- data.frame(mtry = mtry_vector, OOB.error = mean_matrix, 
                   U = ci_matrix[1, ], L = ci_matrix[2, ], 
                   min_mtry = min_mtry)
  
  diagram <- ggplot2::ggplot(df, ggplot2::aes(mtry)) +
    ggplot2::geom_line(ggplot2::aes(y = OOB.error), colour = "blue") +
