...
#'
#' @param expr_df a n x p dataframe used to build the models. The first two columns
...
combinatorialRFMCCV <- function(expr_df,
                                parameters = list(ntrees = 500, nsplits = 100, test_prop = 1 / 3)) {
  START <- proc.time()

  original_data <- expr_df
  start_idx <- 3
  ...
  ### parameters: The label_order param states that 'cases' are 'positives' and 'ctrl' are negatives
  num_of_models <- 1

  ## Here we consider the j-combinations from the col_idx array Each combination represents a set of metabolites. We will consider the expr_dfs
  ## extracted form the original expr_df considering the column indexes correpsonding to these combinations.  On each of these expr_dfs, we will
  ...
  ## RF model with the highest performance on the corresponding expr_df.  To compare each model we will use the Area under the Roc Curve (AUC)
  ...
  for (j in 2:(length(col_idx) - 1)) {
    combinations <- utils::combn(col_idx, j) # a j x n_of_combinations matrix
    num_of_models <- num_of_models + dim(combinations)[2]

    ### Here we try to find the best p
    res <- getBestRFModel(combinations, expr_df, parameters)
