...
#' combination we can extract a expr_df, build a random forest model and perform a cross-validation. We can describe
#' the performance of each cross-validated model with an 'average' ROC curve and its AUC. The collected auc values
#' for each combination (expr_df) are used by the function to build a diagram of the AUC as a function
#' 
...
plotAUCvsCombinations <- function(auc_values, num_of_variables, num_of_combinations) {
  ...
  graphics::points(xmax, ymax, col = "red")
  graphics::abline(v = xmax, h = ymax, col = "red", lty = 3)
  
  graphics::text(x = (xmax + 0.05), y = (ymax + 0.01), labels = paste("AUC max: ", ymax))
  graphics::mtext(text = paste("# var:", num_of_variables), side = 3, line = 2)
  graphics::mtext(text = paste("# comb:", num_of_combinations), side = 3, line = 1)
